...
public enum QuotaFeed {
    ...
    public static func decode(_ data: Data, provider: Provider, now: Date = Date()) throws -> QuotaSnapshot {
        guard data.count <= maxBytes, [.claude, .codex, .gemini, .grok].contains(provider),
        ...
        var seen: Set<String> = []
        let allowedIDs = provider == .claude ? ["Session", "Weekly"] : provider == .codex ? ["Primary", "Secondary"] : provider == .gemini ? ["Pro", "Flash", "Flash Lite"] : ["Weekly"]

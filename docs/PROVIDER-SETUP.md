...
3. After “Quota snapshot published”, choose the quota-feed folder in the dashboard's Codex card. Return in Terminal refreshes; `q` or Ctrl+C stops the official process and publishes unavailable. To use the separate saved login on a later run, open `Bridges/Resume Codex Quotas.command` (or omit `--login`); do not use your normal Codex home. The Resume launcher never starts device sign-in. Failures report only fixed categories, never provider error text. “Saved login unavailable or rejected” means official Codex cannot use that separate saved login; Resume cannot repair it. Start the login launcher and keep the same window open until both “Provider reported login complete.” and “Quota snapshot published.” appear. Binary updates require a fresh reviewed hash, never automatic acceptance.

...
3. After “Quota snapshot published”, choose the quota-feed folder in the dashboard's Codex card. Return in Terminal refreshes; `q` or Ctrl+C stops the official process and publishes unavailable. To use the separate saved login on a later run, open `Bridges/Resume Codex Quotas.command` (or omit `--login`); do not use your normal Codex home. The Resume launcher never starts device sign-in. Failures report only fixed categories, never provider error text. Binary updates require a fresh reviewed hash, never automatic acceptance.
